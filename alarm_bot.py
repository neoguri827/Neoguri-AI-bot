...
ALARM_INSTRUCTION = (
    "너는 뉴스 브리핑 AI다. 반드시 구글 검색을 실제로 호출해 방금 확인한 정보만으로 "
    "아래 항목을 채워라. 검색 없이 추측한 내용은 절대 금지다.\n\n"
...
def _build_briefing_prompt() -> str:
    ...
    return (
        f"지금은 한국 시간 기준 {date_str} {time_str}이다. 이 날짜를 기준으로 경제·사회·국제 등 "
        "다양한 분야의 주요 뉴스 헤드라인을 정리해줘. 검색어에도 이 날짜를 반영해서, "
        "절대 다른 날짜의 옛날 뉴스를 가져오지 마라."
    )

ALARM_WELCOME = (
    "알람너구리 가동\n\n"
    "한국 시간(KST) 기준 매시 정각마다 자동으로 분야별 주요 뉴스를 정리해서 보내드립니다 "
    "(시간당 최대 1회).\n"
